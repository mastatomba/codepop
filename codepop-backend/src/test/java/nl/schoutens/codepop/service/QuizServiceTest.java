...
class QuizServiceTest {
    ...
    void setUp() {
        ...
        Question q1 = new Question(testTopic, "Question 1", "records", Question.Difficulty.EASY, "Explanation 1");
        q1.setId(1L);
        Question q2 = new Question(testTopic, "Question 2", "records", Question.Difficulty.MEDIUM, null);
        q2.setId(2L);
        Question q3 = new Question(testTopic, "Question 3", "streams", Question.Difficulty.HARD, "Explanation 3");
        q3.setId(3L);
    ...
    void getQuiz_shouldThrowException_whenTopicNotFound() {
        ...
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class, 
            () -> quizService.getQuiz("InvalidTopic", null));
    ...
    void getQuiz_shouldCallQuizMaster_whenLessThan5QuestionsAvailable() {
        ...
        when(questionRepository.findByTopicId(1L)).thenReturn(testQuestions);
        when(questionOptionRepository.findByQuestionId(anyLong())).thenReturn(testOptions);
        when(quizMaster.generateQuestions(anyString(), anyInt(), anyList())).thenReturn(Collections.emptyList());
    ...
    void getQuiz_shouldMatchSubtopic_whenUserInputIncludesSubtopic() {
        List<Question> recordsQuestions = Arrays.asList(
    ...
    void getQuiz_shouldReturnEmptyAndCallLLM_whenSubtopicNotFound() {
        ...
        when(questionOptionRepository.findByQuestionId(anyLong())).thenReturn(testOptions);
        when(quizMaster.generateQuestions(anyString(), eq(5), anyList())).thenReturn(Collections.emptyList());
        ...
        verify(quizMaster).generateQuestions(eq("Java nonexistent"), eq(5), anyList());  // Should call LLM
    }
    ...
    void getQuiz_shouldReturnAllTopicQuestions_whenNoSubtopicSpecified() {
        ...
        assertEquals(3, result.totalQuestions());
        verify(questionRepository).findByTopicId(1L);
        verify(questionRepository, never()).findByTopicIdAndSubtopicContainingIgnoreCase(anyLong(), anyString());
    }
}
