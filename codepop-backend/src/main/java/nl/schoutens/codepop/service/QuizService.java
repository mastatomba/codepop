...
public class QuizService {
    ...
    public QuizDTO getQuiz(String userInput, List<Long> excludeQuestionIds) {
        ...
        if (availableQuestions.size() < 5) {
            ...
            List<String> existingQuestionTexts = allQuestions.stream()
            ...
            List<Question> newQuestions = quizMaster.generateQuestions(
                    userInput, 
                    neededCount, 
    ...
    /**
     * Parse user input and find the matching topic in one operation.
     * Avoids duplicate database queries by combining parsing and lookup.
     * 
     * Strategy:
     * 1. Try to match each word in input against topics (exact match)
     * 2. If no match, try fuzzy matching on the full input
     * 3. Return the found topic along with extracted subtopic
     * 
     * Examples:
     *   "Java records" -> Topic(Java), subtopic="records"
     *   "React hooks" -> Topic(React), subtopic="hooks"
     *   "Javascript React" -> Topic(React), subtopic="Javascript"
     *   "Java" -> Topic(Java), subtopic=null
     */
    ...
    /**
     * Fetch questions for a topic, optionally filtered by subtopic.
     * - If subtopic provided: only return questions matching that subtopic (may be empty, triggering LLM)
     * - If no subtopic: return all questions for main topic
     */
